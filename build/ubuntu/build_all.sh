...
sed '/^$/d' "${BUILD_DIR}/targets.txt" | while read _VERSION; do
  ...
  _CMD="docker build --file build/ubuntu/Dockerfile --tag $TAG_NAME --build-arg UBUNTU_VERSION --build-arg PHP_VERSION --compress ."

  if [ "$QUIET" == "false" ]; then
    echo "" >&2
    echo "Building for PHP v$PHP_VERSION on ubuntu v$UBUNTU_VERSION..." >&2
    echo "+ $_CMD" >&2
    echo "" >&2
    $_CMD >&2
  else
